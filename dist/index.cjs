...
var properties = __toESM(require_dist_node11());
...
var VersionObject = class {
  ...
  parse() {
    ...
    for (const key in matcher.groups) {
      if (["major", "minor", "patch"].includes(key)) {
        ...
        this.data[key] = parseInt(matcher.groups[key]) || void 0;
      } else {
        this.data[key] = matcher.groups[key] || void 0;
      }
    }
    this.rawBuild = (_b = this.rawVersion) == null ? void 0 : _b.substr(matcher[0].length);
    if (this.rawBuild) {
      ...
      if (matcher1 && matcher1.groups) {
        ...
        for (const key in matcher1.groups) {
          if (["build"].includes(key)) {
            this.data[key] = parseInt(matcher1.groups[key]) || void 0;
          } else {
            this.data[key] = matcher1.groups[key] || void 0;
          }
        }
      }
      ...
      if (this.rawLabel) {
        ...
        if (matcher2 && matcher2.groups) {
          for (const key in matcher2.groups) {
            this.data[key] = matcher2.groups[key] || void 0;
          }
        }
      }
    }
  ...
  toArray() {
    ...
    vArray.push(this.data.build === void 0 ? BUILD_PREFIX : "");
...
async function run() {
  ...
  try {
    ...
    const github_token = core4.getInput("github_token", { required: false }) || process.env.GITHUB_TOKEN || null;
    const branch = core4.getInput("branch", { required: false });
    const pr = core4.getInput("pr_number", { required: false }) || context.payload.number || null;
    ...
    const prepend_v = core4.getInput("prepend_v", { required: false }) === "true";
    const ignore_v_when_searching = core4.getInput("ignore_v_when_searching", {
      required: false
    }) === "true";
    ...
    try {
      ...
      core4.setFailed(`Action failed with error: ${e}`);
    }
    if (!repos) {
