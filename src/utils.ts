...
export async function getLatestTag(
  ...
  const search_str = ignore_v_when_searching
    ? `^(v)?${escapeRegExp(versionPrefixes.without_v)}`
    : `^${tagPrefix}`;
  ...
  async function createTagList(_fromReleases: boolean): Promise<string[]> {
    ...
    if (_fromReleases) {
      ...
      core.debug(
        `(${Math.round(used * 100) / 100} MB) getLatestTag received tags from releases: found ${
          allNames.length
        }`,
      );
    } else {
