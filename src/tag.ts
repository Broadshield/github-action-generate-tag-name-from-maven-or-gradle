class TagParser {
  cmpTags(a: string, b: string): number {
    return this.cmpArrays(this.tagSortKey(a), this.tagSortKey(b))
  }

  tagSortKey(s: string): (string | number)[] {
    const a: (string | number)[] = s.split(/([0-9]+)/)
  ...
  cmpArrays(a: (string | number)[], b: (string | number)[]): number {
