...
Support boolean input list: \`true | True | TRUE | false | False | FALSE\``);
    ...
    exports.debug = debug5;
    function error3(message, properties2 = {}) {
...
var properties = __toESM(require_dist_node11(), 1);
...
function app_version(path_str) {
  ...
  try {
    ...
    const pomDocument = parser.parse(xmlData);
    ...
    try {
      ...
      core.debug(`ERROR: Property 'version' not found in: (${path_str}) keys available (${pfile.getKeys()})`);
      return void 0;
    } catch (error_) {
      core.error(`ERROR: ${error3}`);
...
// src/versionObject.ts
var LABEL_PREFIX = core2.getInput("label_delimiter", { required: false }) || "-";
var BUILD_PREFIX = core2.getInput("build_delimiter", { required: false }) || "+";
var VersionObject = class {
  constructor(v) {
  ...
  parse() {
    var _a, _b;
    const matcher = (_a = this.rawVersion) == null ? void 0 : _a.match(this.version_regex);
    core2.debug(`parse(): passed ${this.rawVersion}`);
    if (!matcher || matcher.groups === void 0) {
      core2.error(`parse(): Version can't be found in string: ${this.rawVersion}`);
      throw new Error(`Version can't be found in string: ${this.rawVersion}`);
    }
    for (const entry of Object.entries(matcher.groups)) {
      const [key, value] = entry;
      if (["major", "minor", "patch"].includes(key)) {
        this.data[key] = parseInt(value, 10) || 0;
      } else if (["legacy_build_number"].includes(key)) {
        this.data[key] = parseInt(value, 10) || void 0;
      } else {
        this.data[key] = value || void 0;
      }
    }
    this.rawBuild = (_b = this.rawVersion) == null ? void 0 : _b.slice(matcher[0].length);
    if (this.rawBuild) {
      const matcher1 = this.rawBuild.match(this.build_regex);
      let len1 = 0;
      if (matcher1 && matcher1.groups) {
        len1 = matcher1[0].length;
        for (const entry of Object.entries(matcher1.groups)) {
          const [key, value] = entry;
      ...
      this.rawLabel = this.rawBuild.slice(0, Math.max(0, this.rawBuild.length - len1));
      if (this.rawLabel) {
        const matcher2 = this.rawLabel.match(this.label_regex);
        if (matcher2 && matcher2.groups) {
          for (const entry of Object.entries(matcher2.groups)) {
            const [key, value] = entry;
            this.data[key] = value;
          }
        }
    ...
    core2.debug(`parse() created ${JSON.stringify(this.data)}`);
  }
  toArray() {
    const vArray = [];
    vArray.push(this.undfEmpty(this.data.with_v), this.major, this.minor_prefix, this.minor, this.patch_prefix, this.patch, this.undfEmpty(this.data.legacy_build_prefix), this.undfEmpty(this.data.legacy_build_number), this.undfEmpty(this.data.label_prefix), this.undfEmpty(this.data.label), this.data.build === void 0 ? BUILD_PREFIX : "", this.undfEmpty(this.data.build));
    core2.debug(`ToArray() passed ${JSON.stringify(this.data)} returns ${vArray.join("")}`);
    return vArray;
  }
  undfEmpty(vStr) {
    if (vStr === void 0) {
  ...
  bump(bumpType) {
    ...
    switch (bumpType) {
      ...
      case BumpType.Build:
        ...
        break;
      default:
        throw new Error(`BumpType ${bumpType} not supported`);
    }
    return v;
  }
  versionString() {
    const vStr = `${this.data.major}${this.minor_prefix}${this.minor}${this.patch_prefix}${this.patch}`;
    core2.debug(`versionString() returns ${vStr}`);
    return vStr;
  }
  releaseString(display_v) {
    var _a;
    const should_display_v = display_v === void 0 ? ((_a = this.data.with_v) == null ? void 0 : _a.toLowerCase()) === "v" : display_v;
    if (should_display_v) {
  ...
  toString() {
    const vStr = `${this.releaseString()}${this.undfEmpty(this.data.legacy_build_prefix)}${this.undfEmpty(this.data.legacy_build_number)}${this.undfEmpty(this.data.label_prefix)}${this.undfEmpty(this.data.label)}${this.undfEmpty(this.data.build ? BUILD_PREFIX : "")}${this.undfEmpty(this.data.build)}`;
    core2.debug(`toString() passed ${JSON.stringify(this.data)} returns ${vStr}`);
    return vStr;
  }
  prefixString(bumping, suffix, is_release_branch) {
    ...
    switch (bumping) {
      ...
      case BumpType.Build:
        ...
        break;
      default:
        throw new Error(`BumpType ${bumping} not supported`);
    }
    if (this.with_v) {
      result = `${this.undfEmpty(this.with_v)}${result}`;
    }
    const used = process.memoryUsage().heapUsed / 1024 / 1024;
    core2.debug(`(${Math.round(used * 100) / 100} MB) prefixString(): passed ${JSON.stringify(this)} and bumping ${bumping} and returns ${result}`);
    return result;
  }
};
...
function basename(path) {
  ...
  core3.debug(`Basename passed ${path} and returns ${result}`);
  return result;
}
function stripRefs(path) {
  if (!path)
    return null;
  const result = path.replace("refs/heads/", "").replace("refs/tags/", "");
  core3.debug(`stripRefs passed ${path} and returns ${result}`);
  return result;
}
function normalize_version(v_string, default_version = "0.0.1") {
  ...
  const strToNormalize = v_string == null ? void 0 : v_string.trim();
  const match = VERSION_RE.exec(strToNormalize);
  const result = match && match.groups ? match.groups.version : default_version;
  const used = process.memoryUsage().heapUsed / 1024 / 1024;
  core3.debug(`(${Math.round(used * 100) / 100} MB) normalize_version passed ${v_string} with default ${default_version} and returns ${result}`);
...
function repoSplit(inputRepo, context) {
  ...
  if (inputRepo) {
    ...
    core3.debug(`repoSplit passed ${inputRepo} and returns ${JSON.stringify(result)}`);
  } else if (process.env.GITHUB_REPOSITORY) {
    [result.owner, result.repo] = process.env.GITHUB_REPOSITORY.split("/");
    core3.debug(`repoSplit using GITHUB_REPOSITORY ${process.env.GITHUB_REPOSITORY} and returns ${JSON.stringify(result)}`);
...
function bumper(versionObj, bumping, is_release_branch) {
  ...
  core3.debug(`bumper() passed version object ${versionObj.toString()} and bumping ${bumping}}`);
  if (!is_release_branch) {
    core3.debug(`bumper() not in release branch will return ${newVersion.toString()}`);
    return newVersion.toString();
  }
  core3.debug(`bumper() in release branch will return ${newVersion.releaseString()}`);
  return newVersion.releaseString();
}
function getVersionPrefixes(str) {
  const search_re = /^([Vv])?(?<version>.*)/;
  const matcher = str == null ? void 0 : str.match(search_re);
  const used = process.memoryUsage().heapUsed / 1024 / 1024;
  core3.debug(`(${Math.round(used * 100) / 100} MB) getVersionPrefixes passed ${str}`);
...
async function getLatestTag(owner, repo, tagPrefix, fromReleases, sortTags, ignore_v_when_searching, octokit) {
  const usedUp = process.memoryUsage().heapUsed / 1024 / 1024;
  core3.debug(`(${Math.round(usedUp * 100) / 100} MB) getLatestTag passed owner: ${owner}, repo: ${repo}, tagPrefix: ${tagPrefix}, fromReleases: ${fromReleases}, sortTags: ${sortTags}`);
  const pages = {
    owner,
    repo,
    per_page: 100
  ...
  const search_str = ignore_v_when_searching ? `^(v)?${escapeRegExp(versionPrefixes.without_v)}` : `^${tagPrefix}`;
  const search_re = new RegExp(search_str);
  async function createTagList(_fromReleases) {
    let allNames;
    let used = 0;
    if (_fromReleases) {
      allNames = await octokit.paginate(octokit.rest.repos.listReleases, pages, (response) => response.data.map((item) => item.tag_name));
      used = process.memoryUsage().heapUsed / 1024 / 1024;
      core3.debug(`(${Math.round(used * 100) / 100} MB) getLatestTag received tags from releases: found ${allNames.length}`);
    } else {
      allNames = await octokit.paginate(octokit.rest.repos.listTags, pages, (response) => response.data.map((item) => item.name));
      used = process.memoryUsage().heapUsed / 1024 / 1024;
      core3.debug(`(${Math.round(used * 100) / 100} MB) getLatestTag received tags from tags: found ${allNames.length}`);
    }
    return allNames;
  }
  function getMatchedTags(allTagsArray) {
    const tagsList = [];
    for (const tag of allTagsArray) {
      if (search_re.test(tag)) {
        if (!sortTags) {
          core3.debug(`getLatestTag returns ${tag}`);
          try {
        ...
        const used = process.memoryUsage().heapUsed / 1024 / 1024;
        core3.debug(`(${Math.round(used * 100) / 100} MB) getMatchedTags adding tag ${tag}`);
        try {
          tagsList.push(new VersionObject(tag));
        } catch (error3) {
        }
  ...
  if (tags.length === 0) {
    core3.debug(`getLatestTag found 0 tags starting with prefix ${tagPrefix}`);
    return new VersionObject(tagPrefix);
  }
  const usedFin = process.memoryUsage().heapUsed / 1024 / 1024;
  core3.debug(`(${Math.round(usedFin * 100) / 100} MB) getLatestTag found ${tags.length} tags starting with prefix ${tagPrefix}`);
  core3.debug(`getLatestTag found these tags: ${tags.map((t) => t.toString())}`);
  tags.sort(cmpTags);
  const [latestTag] = tags.slice(-1);
  core3.debug(`getLatestTag returns ${latestTag.toString()}`);
  return latestTag;
}
...
async function run() {
  ...
  try {
    ...
    const github_token = core4.getInput("github_token", { required: false }) || process.env.GITHUB_TOKEN || void 0;
    const branch = core4.getInput("branch", { required: false });
    const tmpPr = parseInt(core4.getInput("pr_number", { required: false }) ?? 0, 10);
    const { pull_request } = context.payload;
    const pr = pull_request && tmpPr === 0 ? pull_request.number : tmpPr;
    const filepath = (_a = core4.getInput("filepath", { required: true })) == null ? void 0 : _a.trim();
    const default_version = (_b = core4.getInput("default_version", { required: false })) == null ? void 0 : _b.trim();
    const tag_prefix = (_c = core4.getInput("tag_prefix", { required: false })) == null ? void 0 : _c.trim();
    const releases_only = ((_d = core4.getInput("releases_only", { required: false })) == null ? void 0 : _d.trim()) === "true";
    const sort_tags = ((_e = core4.getInput("sort_tags", { required: false })) == null ? void 0 : _e.trim()) === "true";
    const bump = (_f = core4.getInput("bump", { required: false })) == null ? void 0 : _f.trim().toLowerCase();
    const release_branch = (_g = core4.getInput("release_branch", { required: true })) == null ? void 0 : _g.trim();
    const prepend_v = core4.getBooleanInput("prepend_v", { required: false });
    const ignore_v_when_searching = core4.getBooleanInput("ignore_v_when_searching", {
      required: false
    });
    core4.debug("Loading octokit: started");
    if (!github_token) {
      core4.setFailed("github_token not supplied");
      return;
    }
    const octokit = github.getOctokit(github_token);
    core4.debug("Loading octokit: completed");
    ...
    const is_release_branch = (br == null ? void 0 : br.startsWith(release_branch)) || false;
    const bump_item = !is_release_branch ? "build" : bump;
    const repository = core4.getInput("repository", { required: false }) || process.env.GITHUB_REPOSITORY || null;
    let repos = null;
    try {
      repos = repoSplit(repository, context);
    } catch (error3) {
      core4.setFailed(`Action failed with error: ${error3}`);
    }
    if (!repos) {
      core4.setFailed("Action failed with error: No repository information available");
      return;
    }
    const appVersion = normalize_version(app_version(filepath), default_version);
    core4.debug(`appVersion: ${appVersion}`);
    const prefix = tag_prefix || appVersion;
    let suffix;
    if (pr) {
      suffix = `PR${pr}`;
    } else if (br) {
      suffix = (_h = basename(br)) == null ? void 0 : _h.replace(/\./g, "-");
    }
    const versionObj = new VersionObject(prefix);
    const searchPrefix = versionObj.prefixString(bump_item, suffix, is_release_branch);
    const latestGitTag = await getLatestTag(repos.owner, repos.repo, searchPrefix, releases_only, sortTags, ignore_v_when_searching, octokit);
    const tag_name = bumper(latestGitTag, bump_item, is_release_branch);
    const tagOptions = getVersionPrefixes(tag_name);
    core4.setOutput("tag_name_with_v", tagOptions.with_v);
    core4.setOutput("tag_name_without_v", tagOptions.without_v);
    core4.setOutput("tag_name", prepend_v ? tagOptions.with_v : tagOptions.without_v);
    core4.setOutput("app_version", appVersion);
    core4.setOutput("search_prefix", searchPrefix);
    core4.setOutput("prefix", prefix);
    core4.setOutput("suffix", suffix);
    core4.setOutput("bump_item", bump_item);
    core4.setOutput("latest_git_tag", latestGitTag.toString());
    core4.setOutput("is_release_branch", is_release_branch);
    core4.info(`Tag Name: ${tag_name}`);
    core4.info(`App Version: ${appVersion}`);
    core4.info(`Search Prefix: ${searchPrefix}`);
  } catch (error3) {
    core4.setFailed(`ERROR: ${error3}`);
  }
}
