...
if [ -f .github/scripts/install_prereqs.sh ]; then
    ...
    if [ -f .node-version ]; then
        ...
        ACTION_NODE_VERSION="$(yq -r '.runs.args.NODE_VERSION' action.yml)"
        WORKFLOW_NODE_VERSION="$(yq -r '.jobs.unit_tests.container.image' .github/workflows/run_tests.yml)"
        if [ "${NODE_VERSION}" != "${ACTION_NODE_VERSION}" ]; then
            echo "Updating action.yml Node Version from ${ACTION_NODE_VERSION} to ${NODE_VERSION}"
            yq -i -Y --arg v "${NODE_VERSION}" '.runs.args.NODE_VERSION = $v' action.yml
            git add action.yml
        fi
